...
ARjs.Context = THREEx.ArToolkitContext = function (parameters) {
  ...
  console.assert(
    ["artoolkit", "aruco", "tango"].indexOf(this.parameters.trackingBackend) !==
      -1,
...
ARjs.Context.prototype._updateAruco = function (srcElement) {
  ...
  detectedMarkers.forEach(function (detectedMarker) {
    ...
    _this.arucoContext.updateObject3D(
      tmpObject3d,
      foundControls._arucoPosit,
      foundControls.parameters.size,
      detectedMarker
    );
...
ARjs.Context.prototype._initTango = function (onCompleted) {
  ...
  navigator.getVRDisplays().then(function (vrDisplays) {
    ...
    if (vrDisplay.displayName === "Tango VR Device") {
      _this._tangoContext.vrPointCloud = new THREE.WebAR.VRPointCloud(
        vrDisplay,
        true
      );
    }
...
ARjs.Context.prototype._updateTango = function (srcElement) {
  ...
  var cameraTransformMatrix = new THREE.Matrix4().compose(
    position,
    quaternion,
    scale
  );
